...
def create_parsers():
    ...
    parser = argparse.ArgumentParser(
            description="Build script for minus network setup.",
...
if args.coyote_sw:
    ...
    subprocess.run([SCRIPTS_BUILD_DIR / "coyote-sw.sh", "jigsaw_minus_nw", REBUILD], check=True)

if args.bitstream:
    print("Generating bitstream:")
    subprocess.run([SCRIPTS_BUILD_DIR / "coyote-hw.sh", "jigsaw_minus_nw", REBUILD], check=True)
