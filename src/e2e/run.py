...
COYOTE_HOST_DIR = ROOT_DIR / "submodules" / "Coyote" / "examples" / "jigsaw_host_controller" 
COYOTE_DEVICE_DIR = ROOT_DIR / "submodules" / "Coyote" / "examples" / "jigsaw_device_controller" 

def create_parsers():

    parser = argparse.ArgumentParser(
            description="Run script for end to end setup.",
    ...
    parser_host_proxy.add_argument('-l', '--load_driver', help='Load Coyote Driver', action='store_true')
    parser_host_proxy.add_argument('-r', '--remove_driver', help='Remove Coyote Driver', action='store_true')
    parser_host_proxy.add_argument('-f', '--flash_bitstream', help='Flash the Coyote bitstream', action='store_true')
    parser_host_proxy.add_argument('-s', '--run_software', help='Run the Coyote Software', action='store_true')
...
if args.command == 'host':
    ...
    if args.host_command == 'vm':
        ...
        if args.flash_bitstream:
            ...
            subprocess.run([SCRIPTS_RUN_DIR / "coyote-flash-bitstream.sh", "jigsaw_host_controller"], check=True)
    ...
    if args.flash_bitstream:
        print("Flashing bitstream:")
        subprocess.run([SCRIPTS_RUN_DIR / "coyote-flash-bitstream.sh", "jigsaw_device_controller"], check=True)
