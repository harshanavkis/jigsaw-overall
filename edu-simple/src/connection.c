...
int get_pci_region(disagg_pci_dev_info *vsock_pci_info, uint64_t addr, uint32_t size)
{
    ...
    {
        if ((vsock_pci_info->regions[i].vmPhys) == 0x0)
            continue;
        
        if ((addr >= (vsock_pci_info->regions[i].vmPhys)) && (addr + size <= (vsock_pci_info->regions[i].vmPhys) + *(vsock_pci_info->regions[i].size)))
        {
            return i;
        }
    }
...
void *run_shmem_app(disagg_pci_dev_info *vsock_pci_info, void *opaque) {
    ...
    while (1) {
...
#endif
            cb = vsock_pci_info->regions[pci_region].cb;

            loff_t offset = header.address - (vsock_pci_info->regions[pci_region].vmPhys);
#ifdef CONFIG_DISAGG_DEBUG_MMIO
        ...
        case OP_WRITE:
            ...
            offset = header.address -  (vsock_pci_info->regions[pci_region].vmPhys);
            is_write = true;
	...
	case DISAGG_DEV_OP_DMA_MAP:
            ...
            printf("connection.c: OP_DMA_MAP: Address 0x%lx, Length %u\n", header.address, header.length);
#endif
	    ...
	    printf("dma_map proxy decrypt: %d;%f end\n", header.length, 
...
#ifdef CONFIG_DISAGG_DEBUG_DMA_SEC
	    printf("connection.c: OP_DMA_ENC: Address 0x%lx, Length %u\n", header.address, header.length);
#endif
...
#ifdef CONFIG_DISAGG_DEBUG_DMA_SEC
	    printf("connection.c: OP_DMA_DEC: Address 0x%lx, Length %u\n", header.address, header.length);
#endif
	...
	case DISAGG_DEV_OP_ADDR_INIT:
	    printf("connection.c: OP_ADDR_INIT\n");
	...
	case DISAGG_DEV_OP_BAR_PHYS:
            ...
            if (wait_and_read_data(data, 1) < 0) {
                perror("Failed to read data");
                continue;
            }

	    vsock_pci_info->regions[*((uint8_t *)data)].vmPhys = header.address;

            printf("connection.c: OP_BAR_PHYS: Received bar pyhsical address mapping operation: BAR 0x%x, address 0x%lx\n", *((uint8_t *) data), header.address);
